...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantClasses = {
      ...
      danger:
        'bg-gradient-to-r from-red-500 to-red-600 text-white enabled:hover:from-red-600 enabled:hover:to-red-700 focus:ring-red-500',
      default: '',
    };
    ...
    return (
      ...
      >
        {isLoading ? (
          ...
          <>
            {leftIcon && <figure className="mr-2">{leftIcon}</figure>}
            {children}
            {rightIcon && <figure className="ml-2">{rightIcon}</figure>}
          </>
        )}
      </button>
    );
